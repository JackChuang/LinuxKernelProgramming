...
need_resched_nonpreemptible:
	...
	next = pick_next_task(rq);
        ...
        if(prev->policy==SCHED_CASIO || next->policy==SCHED_CASIO){
                if(prev->policy==SCHED_CASIO && next->policy==SCHED_CASIO){
                        snprintf(msg,CASIO_MSG_SIZE,"SCHED prev->(%d:%d),next->(%d:%d)",prev->casio_id,prev->pid,next->casio_id,next->pid); 
                }
                else{
	...
	if(prev->policy == SCHED_LOTTERY || next->policy == SCHED_LOTTERY){
		if(prev->policy == SCHED_LOTTERY &&  next->policy == SCHED_LOTTERY){
			snprintf(msg1,LOTTERY_MSG_SIZE,"CONTEX prev->(%d:%llu),next->(%d:%llu)",
                                        prev->pid, prev->ticket, next->pid, next->ticket);
		} else {
			if(prev->policy==SCHED_LOTTERY){
                snprintf(msg1,LOTTERY_MSG_SIZE,"SCHED prev->(%d:%llu),next->(%d:-1)",
                                                    prev->pid, prev->ticket, next->pid);
            } else {
                snprintf(msg1,LOTTERY_MSG_SIZE,"SCHED prev->(%d:-1),next->(%d:%llu)",
                                                    prev->pid, next->pid, next->ticket);
            }
		}//END else
		register_lottery_event( sched_clock(), msg1, LOTTERY_CONTEXT_SWITCH);
	}
#endif 
				...
				struct sched_param *param, bool user)
{
	...
	int reset_on_fork;
            ...
            printk("%s(): RB_lottery\n", __func__);
#else
            printk("%s(): Linkedlist_lottery\n", __func__);
#endif
            printk("%s(): before: p->prio %d  static_prio %d normal_prio %d rt_priority %d\n",
                                __func__, p->prio, p->static_prio, p->normal_prio, p->rt_priority);
#endif
...
recheck:
	...
	__setscheduler(rq, p, policy, param->sched_priority);
	   ...
	   if(policy == SCHED_LOTTERY){
			...
			p->lottery_task.task = p;
            p->ticket = param->sched_priority;
			p->lottery_task.ticket = p->ticket;
            printk("%s(): after: p->pid %d ticket %llu #oftickets %llu\n",
                                __func__, p->pid, p->ticket, p->ticket);
            printk("%s(): after: p->prio %d  static_prio %d normal_prio %d rt_priority %d\n",
                                __func__, p->prio, p->static_prio, p->normal_prio, p->rt_priority);
	   }
#endif
